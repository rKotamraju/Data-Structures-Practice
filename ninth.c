...
int delete_node(int num){ 
	...
	if(searchPREV == NULL){
		...
		if(ptr->right == NULL && ptr->left == NULL){
			if(side == 0){ //if node to be deleted is a leaf with no children
			...
			//printf("1 child - left\n");
			if(ptr->value == root->value){ //if root
			...
			else if(searchPREV->value < ptr->left->value){
